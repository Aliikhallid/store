...
import GroundFloor from '../../assets/viola-groundfloor.webp'
import FirstFloor from '../../assets/viola-firstfloor.webp'
import { useLayoutEffect } from "react";
...
const [title, setTitle] = useState('Ground Floor (123.5)M2')
...
const [t5, setT5] = useState('Master Bedroom');
const [t6, setT6] = useState('Bedroom');
const [t8, setT8] = useState('WC');
const [t9, setT9] = useState('Garage');
const [d1, setD1] = useState('5.0 M x 5.5 M');
const [d2, setD2] = useState('3.8 M x 5.5 M');
const [d3, setD3] = useState('2.7 M x 2.1 M');
const [d4, setD4] = useState('4.0 M x 3.6 M');
const [d5, setD5] = useState('4.8 M x 3.9 M');
const [d6, setD6] = useState('3.4 M x 3.6 M');
const [d8, setD8] = useState('1.1 M x 1.8 M');
const [d9, setD9] = useState('--');
  ...
  const setOver = () => {
    ...
    setShoww(true)
    setOverview('')
    setTitle('Ground Floor (123.5)M2')
    setOverview2('Rooms')
    setT1('Reception')
 setT2('Living Room');
 setT3('Bathroom');
 setT4('Kitchen');
 setT5('Master Bedroom');
 setT6('Bedroom');
 setT8('WC');
 setT9('Garage');
 setD1('5.0 M x 5.5 M');
...
const loadImages = (project: string,lastImage:number) => {
  ...
  return (
    <div className="project-page flex flex-col h-fit t-10 bg-white" dir={`${isRTL? "rtl":"ltr"}`}>
      ...
      <div
        ...
        <p className="md:text-[64px] text-[12px]">Viola</p>
        </div>
        <p className="md:max-w-[75rem] text-[22px]  light text-md max-w-[40rem]">The most popular Villas in the compound, which were specially designed for families who are looking for a comfortable Villa with an elegant design, and has a parking space for a car, green yard and a large balcony.</p>
  ...
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Number of villas</p> <p className="md:text-[14px] text-[10px]">571</p> </div>
  ...
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Land Surface</p> <p className="md:text-[14px] text-[10px]">220 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Ground floor</p> <p className="md:text-[14px] text-[10px]">123.5 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Building area</p> <p className="md:text-[14px] text-[10px]">153.3 m2</p> </div>
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Bedrooms</p> <p className="md:text-[14px] text-[10px]">3</p> </div>
