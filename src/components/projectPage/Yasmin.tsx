...
import GroundFloor from '../../assets/yasmin-groundfloor.webp'
import FirstFloor from '../../assets/yasmin-firstfloor.webp'
import { useLayoutEffect } from "react";
...
const [title, setTitle] = useState('Ground Floor (130.10)M2')
...
const [d1, setD1] = useState('6.2 M x 4.2 M');
const [d2, setD2] = useState('3.2 M x 5.2 M');
const [d3, setD3] = useState('2.4 M x 1.6 M');
const [d4, setD4] = useState('3.5 M x 5.5 M');
const [d5, setD5] = useState('1.3 M x 2.1 M');
const [d6, setD6] = useState('3.7 M x 4.0 M');
const [d8, setD8] = useState('4.3 M x 4.0 M');
const [d9, setD9] = useState('1.8 M x 1.2 M');
  ...
  const setOver = () => {
    ...
    setShoww(true)
    setOverview('')
    setTitle('Ground Floor (130.10)M2')
    setOverview2('Rooms')
    setT1('Reception')
 setT2('Living Room');
 setT3('Bathroom');
...
const loadImages = (project: string,lastImage:number) => {
  ...
  return (
    <div className="project-page flex flex-col h-fit t-10 bg-white" dir={`${isRTL? "rtl":"ltr"}`}>
      ...
      <div
        ...
        <p className="md:text-[64px] text-[12px]">Yasmin</p>
        </div>
        <p className="md:max-w-[75rem] text-[22px] light text-md max-w-[40rem] light">Stylish and comfortable, with parking areas, green yard, balcony, and three master bedrooms with great interior and exterior design.</p>
  ...
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Number of villas</p> <p className="md:text-[14px] text-[10px]">172 Yasmin + 38 Yasmin corner</p> </div>
  ...
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Land Surface</p> <p className="md:text-[14px] text-[10px]">260 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Ground floor</p> <p className="md:text-[14px] text-[10px]">130.10 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">First floor</p> <p className="md:text-[14px] text-[10px]">33.20 m2</p> </div>
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Building area</p> <p className="md:text-[14px] text-[10px]">153.3 m2</p> </div>
    ...
    <div className="flex flex-col h-fit w-full py-5 justify-center items-center text-center">
      ...
      <ImageSlider imgUrls={loadImages("Viola",2)} imgOverText={["Living Room","Football Turf","Streets"]} />
    ...
    <div className="flex flex-col h-fit w-fit">
      <div className="flex justify-center items-center regular text-xl text-primary-color mt-10">Yasmin</div>
