...
import GroundFloor from '../../assets/narjis-groundfloor.webp'
import FirstFloor from '../../assets/narjis-firstfloor.webp'
import { useLayoutEffect } from "react";
...
const [d1, setD1] = useState('4.0 M x 5.0 M');
const [d2, setD2] = useState('3.1 M x 3.6 M');
const [d3, setD3] = useState('2.0 M x 1.6 M');
const [d4, setD4] = useState('3.0 M x 4.5 M');
const [d5, setD5] = useState('1.0 M x 2.3 M');
const [d6, setD6] = useState('3.3 M x 3.8 M');
const [d8, setD8] = useState('3.3 M x 3.8 M');
const [d9, setD9] = useState('1.0 M x 1.4 M');
  ...
  const setOver = () => {
    ...
    setShoww(true)
    setOverview('')
    setTitle('Ground Floor (32.20)M2')
    setOverview2('Rooms')
    setT1('Reception')
 setT2('Living Room');
 setT3('Bathroom');
...
const loadImages = (project: string,lastImage:number) => {
  ...
  return (
    <div className="project-page flex flex-col h-fit t-10 bg-white" dir={`${isRTL? "rtl":"ltr"}`}>
      ...
      <div
        ...
        <p className="md:text-[64px] text-[12px]">Narjis</p>
        </div>
        <p className="md:max-w-[75rem] text-[22px]  light text-md max-w-[40rem]">Narjis Villa is the best economic choice. It has three bedrooms, a comfortable living room, a parking space for one car, a green yard and a beautiful balcony on the first floor.</p>
  ...
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Number of villas</p> <p className="md:text-[14px] text-[10px]">132 Narjis + 24 Narjis corner</p> </div>
  ...
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Land Surface</p> <p className="md:text-[14px] text-[10px]">200 m2 Narjis, 106 m2 Narjis Corner</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Ground floor</p> <p className="md:text-[14px] text-[10px]">106 m2 Narjis, 26.7 m2 Narjis Corner</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">First floor</p> <p className="md:text-[14px] text-[10px]">26.7 m2 Narjis, 11.8 m2 Narjis Corner</p> </div>
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Building Area</p> <p className="md:text-[14px] text-[10px]">132.7 m2 Narjis, 132.3 m2 Narjis Corner</p> </div>
    ...
    <div className="flex flex-col h-fit w-full py-5 justify-center items-center text-center">
      ...
      <ImageSlider imgUrls={loadImages("Narjis",2)} imgOverText={["Football Turf","Streets","Living Room"]} />
    ...
    <h1 className='regular text-md relative mt-40'>DEMONSTRATION ~ NARJIS</h1>
