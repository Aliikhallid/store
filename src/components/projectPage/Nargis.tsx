...
import GroundFloor from '../../assets/nargis-groundfloor.webp'
import FirstFloor from '../../assets/nargis-firstfloor.webp'
import { useLayoutEffect } from "react";
...
const [title, setTitle] = useState('Ground Floor (129.48)M2')
...
const [d1, setD1] = useState('4.4 M x 5.2 M');
const [d2, setD2] = useState('3.0M x 5.8M');
const [d3, setD3] = useState('5.0M x 3.0M');
const [d4, setD4] = useState('2.4 M x 5.2 M');
const [d5, setD5] = useState('1.3 M x 4.1 M');
const [d6, setD6] = useState('5.0 M x 3.4 M');
const [d7, setD7] = useState('4.4 M x 3.4 M');
const [d8, setD8] = useState('1.4 M x 1.4 M');
  ...
  const setOver = () => {
    ...
    setShoww(true)
    setOverview('')
    setTitle('Ground Floor (129.48)M2')
    setOverview2('Rooms')
    setT1('Reception')
 setT2('Living Room');
 setT3('Kitchen');
...
const loadImages = (project: string,lastImage:number) => {
  ...
  return (
    <div className="project-page flex flex-col h-fit t-10 bg-white" dir={`${isRTL? "rtl":"ltr"}`}>
      ...
      <div
        ...
        <p className="md:text-[64px] text-[12px]">Nargis</p>
        </div>
        <p className="md:max-w-[75rem] text-[22px]  light text-md max-w-[40rem]">Villas with a modern pattern, that were especially designed to families who are looking for a comfortable housing, characterized by an elegant design, with parking spaces for two cars, green yard and large balcony.</p>
  ...
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Number of villas</p> <p className="md:text-[14px] text-[10px]">219</p> </div>
  ...
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Land Surface</p> <p className="md:text-[14px] text-[10px]">250 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">Ground floor</p> <p className="md:text-[14px] text-[10px]">129.48 m2</p> </div>
  <div className={`p-4 relative ${isRTL? "border-l":"border-r"} border-white flex flex-col gap-1 justify-center`}><p className="md:text-[16px] text-[14px] text-primary">First floor</p> <p className="md:text-[14px] text-[10px]">30.78 m2</p> </div>
  <div className="p-4 relative  flex flex-col gap-1 justify-center"><p className="md:text-[16px] text-[14px] text-primary">Building area</p> <p className="md:text-[14px] text-[10px]">160.26 m2</p> </div>
    ...
    <div className="flex flex-col h-fit w-full py-5 justify-center items-center text-center">
      ...
      <ImageSlider imgUrls={loadImages("Nargis",3)} imgOverText={["Playground","Streets","Enterance","Outdoor"]} />
    ...
    <div className="flex flex-col h-fit w-fit">
      <div className="flex justify-center items-center regular text-xl text-primary-color mt-10">Nargis</div>
